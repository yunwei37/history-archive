...
第一：安静的环境和时间很重要，开发很需要时间如果太忙碌就可以考虑换一天开发，一两个小时或者就几个小时是很难有充足时间完全开发成功。开发需要一鼓作气的高潮了才能真正的走上开发的道路。环境自然就不用多说被打扰就完蛋了（可以考虑旅馆住一天）
